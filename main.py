...
canvas = cv2.rectangle(canvas, (275, 1), (370, 65),
                       (0, 255, 0), 2)  # green box
canvas = cv2.rectangle(canvas, (390, 1), (485, 65), (0, 0, 255), 2)  # red box
canvas = cv2.rectangle(canvas, (505, 1), (600, 65),
                       (0, 255, 255), 2)  # yellow box

# label the canvas boxes
cv2.putText(canvas, "CLEAR", (49, 33), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 2, cv2.LINE_AA)
cv2.putText(canvas, "BLUE", (185, 33), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 2, cv2.LINE_AA)
cv2.putText(canvas, "GREEN", (298, 33), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 2, cv2.LINE_AA)
cv2.putText(canvas, "RED", (420, 33), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 2, cv2.LINE_AA)
cv2.putText(canvas, "YELLOW", (520, 33), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 2, cv2.LINE_AA)
...
while ret:
    ...
    # rgb format used for processing hands
        ...
        for j in range(len(points[i])):
            for k in range(1, len(points[i][j])):
                if points[i][j][k-1] is None or points[i][j][k] is None:
                    continue
                cv2.line(frame, points[i][j][k-1], points[i]
                         [j][k], colors[i], line_thickness)
                cv2.line(canvas, points[i][j][k-1], points[i]
                         [j][k], colors[i], line_thickness)
    ...
    if key == ord('q'):
        ...
        line_thickness += 1
    elif key == ord('-') and line_thickness > 1:  # Decrease line thickness (minimum 1)
        line_thickness -= 1
    elif key == ord('s'):  # Save the canvas as an image
        cv2.imwrite("my_artwork.png", canvas)
